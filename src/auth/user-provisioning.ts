...
import { reportResources } from "./types.js";
...
// Shared whitelist upsert used by the CLIs (db:add-user, db:whitelist-batch)
// and — when explicitly enabled — the in-band WhatsApp admin command. Keeping
// one implementation means the encryption, hashing, permission reconciliation,
// and audit chaining stay identical no matter how a user is onboarded.
...
// a failing row in a batch is identifiable (e.g. "row 3: name must be...").
